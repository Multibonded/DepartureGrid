...
for windex in range(25):
    ...
    data=pd.read_csv('{}{}_lines_yong'.format(element,ion),dtype='str')
            ...
            for f in fg:
                for x in xg:
                    for a in ag:
                        ...
                        if i.size:
                            if lte_ew[i] > 1E-4:
                                reduced = np.log10(float(lte_ew[i])/float(line))
                                if reduced > -4.9:
                                    """ind += 1
                        ...
                        if i.size:
                            if nlte_ew[i] > 1E-4:
                                reduced = np.log10(float(nlte_ew[i]) / float(line))
                                if reduced > -4.9:
                                    """ind += 1
    ...
    print(tg.shape)
    outfile=open("grids/{}{}/{}{}_{}_yong.pkl".format(element, ion, element,ion,wg[windex]),"wb")
