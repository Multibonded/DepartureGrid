...
def plot_basic_corrections(element):
    b = pkl.load(open(element + "_Galah_impacts2.pkl", "rb"))
    a = pkl.load(open(element + "_NLTE_list2.pkl", "rb"))
...
def color_single(element):
    b = pkl.load(open(element + "_Galah_impacts2.pkl", "rb"))
    a = pkl.load(open(element + "_NLTE_list2.pkl", "rb"))
    c = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))
...
def scatter():
    ...
    for element in elementlist:
        ...
        if load:
            ...
            linedict = pkl.load(open(element + "_Galah_impacts2.pkl", "rb"))
        ...
        plt.savefig("graphs/"+element+"_STD2.png", bbox_inches="tight")
        plt.show()
...
def ion_imbalance():
    ...
    for element in ["Ti1"]:
        ...
        impact = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))
        if element == "Ti1":
            alt_impact = pkl.load(open("Ti2_Galah_impacts_lineless2.pkl", "rb"))
        else:
            alt_impact = pkl.load(open("Ti1_Galah_impacts_lineless2.pkl", "rb"))
        nlte_imbalance_list = []
        lte_imbalance_list = []
        logg_list = []
        feh_list = []
...
def excitation_balance():
    for lteval in [False, True]:
        ...
        elementstr = "Ti I"
        linedict = pkl.load(open(element + "_Galah_impacts2.pkl", "rb"))
        lineless = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))
...
def hr_diag():
    for element in ["Ti1", "Ti2"]:
        ...
        plt.savefig("graphs//"+str(element)+"HRdiagNLTE2.png", bbox_inches="tight")
        plt.show()
...
def color_single_fe():
    ...
    for element in ["Ti1", "Ti2"]:
        ...
        if reduced:
            b = pkl.load(open(element + "_Galah_impacts2_reduced.pkl", "rb"))
            a = pkl.load(open(element + "_NLTE_list2_reduced.pkl", "rb"))
            c = pkl.load(open(element + "_Galah_impacts_lineless2_reduced.pkl", "rb"))
        else:
            b = pkl.load(open(element + "_Galah_impacts2.pkl", "rb"))
            a = pkl.load(open(element + "_NLTE_list2.pkl", "rb"))
            c = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))
        k = [i for i in b.keys()]
        bc = pkl.load(open("Ti2_Galah_impacts2.pkl", "rb"))
        k2 = [i for i in bc.keys()]
        kk = k + (k2)
        print("number of stars", len(set(kk)))


        findmax = False
        if findmax:
            print(max(a[1]))
            print(np.where(a[1]==max(a[1])))
            ab = map(abs, a[1])
            print(np.where(a[1]==min(ab)))
            print(a[0][21418], a[1][21418], list(c.keys())[21418], list(c.keys())[32222])
            print(c[389271], c[625548])
            exit()
        hr = ((np.where(np.isnan(a[1])))[0])
        fehgiants = []
        fehdwarfs = []
    ...
    plt.savefig("graphs/NLTEimpact_Colormap_Fe_2.png", bbox_inches="tight")
    plt.show()
def scatter_fe():
    elementlist  = ["Ti2", "Ti1"]
    fig = plt.figure()
    ...
    for element in elementlist:
        ...
        if load:
            ...
            linedict = pkl.load(open(element + "_Galah_impacts2_reduced.pkl", "rb"))
            a = pkl.load(open(element + "_NLTE_list2.pkl", "rb"))
            c = pkl.load(open(element + "_Galah_impacts_lineless2_reduced.pkl", "rb"))
            feh = np.asarray([i[2] for i in c.values()])
            lte = np.asarray([i[-2] for i in c.values()])
    ...
    plt.savefig("graphs/STD_fe2.png")
    plt.show()
import pandas as pd
...
def galactic_evolution_trendlines_ionmean():
    ...
    for element in ["Ti1", "Ti2"]:
        trenddict[element] = {}
        for lteval in [False, True]:
            lineless = pkl.load(open(element + "_Galah_impacts_lineless2_reduced.pkl", "rb"))
            ...
            # Checking every 0.2 metallicity bin so we have a good averaged trend line
            steps = 0.2
            for y in np.arange(min(feh), max(feh), steps):
                windwarf = \
                np.where(np.logical_and(y - steps/2 <= feh, feh <= y + steps/2))[0]
                winmeandwarf = np.mean(np.asarray(abundance)[windwarf])
                if not len(windwarf) < 30:
                    trendfehdwarfs.append(y)
                    trendnltedwarfs.append(winmeandwarf)
...
def galactic_evolution_trendlines_fe():
    ...
    for element in ["Ti1", "Ti2"]:
        ...
        for lteval in [False, True]:
            ...
            for y in np.arange(min(feh), max(feh), steps):
                windwarf = \
                np.where(np.logical_and(np.logical_and(y - steps/2 <= feh, feh <= y + steps/2), np.asarray(logg) > 3.5))[0]
                winmeandwarf = np.mean(np.asarray(abundance)[windwarf])
                if not len(windwarf) < 30:
                    trendfehdwarfs.append(y)
                    trendnltedwarfs.append(winmeandwarf)
...
def galactic_evolution_trendlines_fe_line(wl):
    ...
    for lteval in [False, True]:
        lined = pkl.load(open(element + "_Galah_impacts2_reduced_all.pkl", "rb"))
        a = pkl.load(open(element + "_NLTE_list2_reduced_all.pkl", "rb"))
        lineless = pkl.load(open(element + "_Galah_impacts_lineless2_reduced_all.pkl"   , "rb"))
...
def ion_imbalance_fe_line():
    ...
    for element in ["Ti1"]:
        ...
        # Teff logg feh vmic lteabund nlteIMPACT
        impact_orig = pkl.load(open(element + "_Galah_impacts2.pkl", "rb"))
        impact = pkl.load(open(element + "_Galah_impacts2.pkl", "rb"))
        # just roll with it, it's intentional
        if element == "Ti1":
            alt_impact = pkl.load(open("Ti2_Galah_impacts2.pkl", "rb"))
...
def galactic_evolution_trendlines_fe_line_removal():
    ...
    for element in ["Ti2", "Ti1"]:
        ...
        for lteval in [False, True]:
            print("LTE?", lteval)
            lined = pkl.load(open(element + "_Galah_impacts2.pkl", "rb"))
            a = pkl.load(open(element + "_NLTE_list2.pkl", "rb"))
            lineless = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))



            lineless_original = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))
            lineless = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))
...
def galactic_evolution_trendlines_fe_splitup():
    ...
    for element in ["Ti1", "Ti2"]:
        ...
        for lteval in [False, True]:
            ...
            for y in np.arange(min(feh), max(feh), steps):
                windwarf = \
                np.where(np.logical_and(np.logical_and(np.logical_and(y - steps/2 <= feh, feh <= y + steps/2), np.asarray(logg) > 3.5), np.asarray(vmic) != 1.114))[0]
                winmeandwarf = np.mean(np.asarray(abundance)[windwarf])
                if not len(windwarf) < 30:
                    trendfehdwarfs.append(y)
                    trendnltedwarfs.append(winmeandwarf)
...
def ion_imbalance_fe_split():
    ...
    for element in ["Ti1"]:
        ...
        if not reduced:
            impact_orig = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))
            impact = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))
            # just roll with it, it's intentional
            if element == "Ti1":
                alt_impact = pkl.load(open("Ti2_Galah_impacts_lineless2.pkl", "rb"))
            ...
            impact_orig = pkl.load(open(element + "_Galah_impacts_lineless2_reduced.pkl", "rb"))
            impact = pkl.load(open(element + "_Galah_impacts_lineless2_reduced.pkl", "rb"))
            # just roll with it, it's intentional
            if element == "Ti1":
                alt_impact = pkl.load(open("Ti2_Galah_impacts_lineless2_reduced.pkl", "rb"))
        ...
        for starn in impact:
            if starn in alt_impact:
                ...
                lte_imbalance_list.append(ti1lte-ti2lte)
                logg_list.append(impact[starn][1])
                teff_list.append(impact[starn][0])
                if verbose:
                    if impact[starn][2] < -2:
                        # [teff, logg, feh, xi, ltemean, nlte mmean impact]
                        print("LTE, NLTE impact, NLTE abund of Ti I:   ", impact[starn][-2],  impact[starn][-1], impact[starn][-2]+ impact[starn][-1])
        ...
        for lteval in [True, False]:
            ...
            for y in np.arange(min(feh_list), max(feh_list), 0.2):
                windwarf = np.where(
                        ...
                        np.logical_and(
                        np.logical_and(
                            y - 0.1 <= feh_list, feh_list <= y + 0.1), np.asarray(logg_list) > 3.5), np.asarray(teff_list) < 5500))[0]
                ...
                np.where(
                    ...
                    np.logical_and(
                    np.logical_and(
                        y - 0.1 <= feh_list, feh_list <= y + 0.1), np.asarray(logg_list) <= 3.5), np.asarray(teff_list) < 5500)
                    )[0]
            ...
            for star in ['giant', 'dwarf']:
                ...
                if star == "giant":
                    ...
                    if lteval:
                        ...
                        plt.xlabel("Metallicity [Fe/H]")
                    df = pd.DataFrame(dict(x=dwarffeh, y=dwarfion))
                    cmapsuse = LinearSegmentedColormap.from_list("white_viridis", [
                        (0, '#ffffff'),
                        (1e-20, '#1d27cd'),
                        (0.3, '#1d7dcd'),
        ...
        #plt.legend()
        plt.savefig("graphs/Comb_ionimb2.png", bbox_inches="tight")
        plt.show()
...
def galactic_evolution_trendlines_fe_splitup_testheat():
    ...
    #plt.title("Titanium abundance trend lines")
    #plt.colorbar(den, label="Number of stars")
    plt.colorbar(dsartist, cax=cbar_ax)
...
def hr_diag_rev():
    for element in ["Ti1"]:
        ...
        plt.xlabel("Log($g$)")
        plt.savefig("graphs/HRdiagNLTE2_reversed.png", bbox_inches="tight")
        plt.show()

def galactic_evolution_trendlines_ionmeantest():
    trenddict = {}
    errordict = {}
    for element in ["Ti1", "Ti2"]:
        trenddict[element] = {}
        ...
        for lteval in [False, True]:
            ...
            for y in np.arange(min(feh), max(feh), 0.1):
                ...
                if not len(windwarf) < 30:
                    trendfehdwarfs.append(y)
                    trendnltedwarfs.append(winmeandwarf)
                    # Error bars must be created in a dictionary form here with the same size and shape, as we plot only
                    # a 2d list for each lte/ion with a mean value, removing the possibility of std that represents the
                    # entire plot. I think.
                    trenderror.append(winmeandwarf+std(abundance[windwarf]))
                    trendlowerror.append(winmeandwarf-std(abundance[windwarf]))
            # plt.plot(trendfehdwarfs, trendnltedwarfs, c="black", label = "Dwarf (N="+str(dwarfcount)+")")
            # plt.plot(trendfehgiants, trendnltegiants, "--",  c="blue", label = "Giant (N="+str(giantcount)+")")

            trenddict[element][lteval] = [trendnltedwarfs, trendfehdwarfs]
            errordict[element][lteval] = [trenderror, trendlowerror]
    ...
    for element in ["Ti1", "Ti2"]:
        for lteval in [True, False]:
            ...
            abund = [x for _,x in sorted(zip(trenddict[element][lteval][1], trenddict[element][lteval][0]))]
            error = [x for _,x in sorted(zip(trenddict[element][lteval][1], errordict[element][lteval][0]))]
            lowerror = [x for _,x in sorted(zip(trenddict[element][lteval][1], errordict[element][lteval][1]))]

            met = sorted(trenddict[element][lteval][1])
            df = pd.DataFrame(dict(met=met, abund=abund))

            sns.lineplot(data=df, x ="met", y= "abund", errorbar="ci", err_style="band", color=col)
            # plots the error bars
            plt.plot(met, error, color=col, alpha=0.1)
            plt.plot(met, lowerror, color=col, alpha=0.1)
            plt.fill_between(met, error, lowerror, color=col,alpha=0.2)
            #sns.lineplot(x=trenddict[element][lteval][1], y=trenddict[element][lteval][0], label=elementstring+" "+ ltestring, color = col, errorbar = "sd")
...
def color_single_fe_teff():
    ...
    for element in ["Ti1", "Ti2"]:
        ...
        print(len(a[0]))
        print(len(b))
        print("size of fig in pixels:" , get_ax_size(ax,fig))
        ...
        for y in np.arange(min(teff), max(teff) , steps):
            ...
            if not len(wingiant) < 30:
                print(len(a[1]), len(wingiant), len(logg), len(teff), (y))
                winmeangiant = np.mean(np.asarray(a[1])[wingiant])
    ...
    plt.savefig("graphs/NLTEimpact_Colormap_Teff_2.png", bbox_inches="tight")
    plt.show()
def color_single_fe_logg():
    ...
    plt.savefig("graphs/NLTEimpact_Colormap_Logg_2.png", bbox_inches="tight")
    plt.show()
...
def linescatter():
    ...
    for element in ["Ti1", "Ti2"]:
        ...
        for startype in ["dwarf", "giant"]:
            ...
            ax.axis(xmin = -1.8, xmax = 0.5, ymin = 0.06, ymax = 0.24)
            ...
            for line in lines:
                ...
                if element == "Ti1":
                    if "dwarf" in startype:
                        sns.lineplot(x=trendfehdwarfs, y=stdltedwarfs, label=linestr + "$\mathrm{\AA}$", c=colors[where], ax = ax)
                        sns.lineplot(x=trendfehdwarfs, y=stdnltedwarfs, linestyle="--", c=colors[where], ax = ax)

                    else:
                        sns.lineplot(x=trendfehgiants, y=stdltegiants, label=linestr + "$\mathrm{\AA}$", c=colors[where], ax = ax)
            ...
            if "dwarf" in startype:
                ...
                if "1" in element:
                    plt.title("Giants")
    plt.savefig("graphs/linescatter.png", bbox_inches="tight")
    plt.show()
def hr_diag_absolute():
    ...
    for element in ["Ti1", "Ti2"]:
        ...
        # Teff logg feh vmic lteabund nlteIMPACT
        impact = pkl.load(open(element + "_Galah_impacts_lineless2_Ati_reduced.pkl", "rb"))
        impact2 = pkl.load(open(element + "_Galah_impacts_lineless2.pkl", "rb"))
        nlteimpact = []
        lte_imbalance_list = []
        logg_list = []
        feh_list = []
        ...
        if "1" in element:
            plt.ylabel("log(g / cm s$^{-2}$)")
        plt.savefig("graphs//"+str(element)+"HRdiagNLTE2_absolute.png", bbox_inches="tight")
        plt.show()
def print_total_stars():
    element = "Ti2"
    ...
    kk = k + (k2)
    print("number of stars", len(set(kk)))
    pkl.dump(kk, open("final_stars_used.pkl", "wb"))

def galactic_evolution_trendlines_fe_splitup_black():
    trenddict = {}
    fig = plt.figure()
    fig.set_figwidth(15)
    fig.set_figheight(10)
    ...
    plt.savefig("graphs/Split_GCE_Trendlines_fe2_black.png", bbox_inches="tight")
    plt.show()



def ion_imbalance_fe_split_black():
    verbose = True
    heatmap = True
    fig = plt.figure()
    fig.set_figwidth(15)
    ...
    for element in ["Ti1"]:
        ...
        for lteval in [True, False]:
            ...
            for y in np.arange(min(feh_list), max(feh_list), 0.2):
                ...
                if not len(windwarf) < 30:
                    trendfehdwarfs.append(y)
                    trenddwarfs.append(winmeandwarf)
                    dwawrfstd.append(np.std(winmeandwarf))
                    if verbose:
                        print("dwarf, lteval?, metallicity, mean imbalance, number of stars", lteval, y, winmeandwarf, len(windwarf))
                ...
                if not len(wingiant) < 30:
                    ...
                    trendfehgiants.append(y)
                    trendgiants.append(winmeangiant)
                    giantstd.append(np.std(winmeangiant))
                    if verbose:
                        print("giant, lteval?, metallicity, mean imbalance, number of stars", lteval, y, winmeangiant, len(wingiant))
            print(dwarfion, "\n", dwarffeh)
            if not heatmap:
        ...
        #pkl.dump([feh_list, nlte_imbalance_list, lte_imbalance_list, logg_list], open("ion_imbalance_feh_nlte_lte2.pkl", "wb"))
            ...
            for star in ['giant', 'dwarf']:
                ...
                if star == "giant":
                    ...
                    ax.set_ylim([-0.6, 0.6])
                    ax.hlines(0, -2.5, 0.5, linestyles="--", color="grey", linewidth =2)
                    pddf = pd.DataFrame(trendfehgiants, trendgiants)
                    sns.lineplot(data=pddf, x=trendfehgiants, y=trendgiants, linestyle="-", color=clr, label = ltestring+" giants ("+str(len(giantfeh))+")", linewidth =2,errorbar='sd')
                    ...
                    sns.lineplot(x=trendfehdwarfs, y=trenddwarfs, linestyle="-", color=clr, ax=ax, label = ltestring+ " dwarfs ("+ str(len(dwarffeh))+")",errorbar='sd')
        ...
        #plt.legend()
        plt.savefig("graphs/Comb_ionimb2_black.png", bbox_inches="tight")
        plt.show()
        ...
        for startype in ["dwarf", "giant"]:
            ...
            if element == "Ti1":
                ...
                if "dwarf" in startype:
                    ...
                    ax = fig.add_subplot(2, 2, 4)
            ax.axis(xmin=-2, xmax=0.5, ymin=-0.2, ymax=0.4)
            ...
            for line in lines:
                ...
                for y in np.arange(min(feh[line]), max(feh[line]), steps):
                    windwarf = np.where(
                        np.logical_and(np.logical_and(y - (steps / 2) <= feh[line], feh[line] <= y + (steps / 2)),
                                       np.asarray(logg[line]) > 3.5))[0]


                    if len((np.asarray(linedict['lte'][line])[windwarf])) < 2:
                        continue
                    ltefeh = np.asarray(linedict['lte'][line])-np.asarray(feh[line])
                    nltefeh = np.asarray(linedict['nlte'][line])-np.asarray(feh[line])
                    stddwarf = np.mean(np.asarray(ltefeh)[windwarf])
                    stddwarfnlte = np.mean(np.asarray(nltefeh)[windwarf])
                    ...
                    stdgiant = np.mean(np.asarray(ltefeh)[wingiant])
                    stdgiantnlte = np.mean(np.asarray(nltefeh)[wingiant])
                ...
                if element == "Ti1":
                    if "dwarf" in startype:
                        sns.lineplot(x=trendfehdwarfs, y=stdltedwarfs, label=linestr + "$\mathrm{\AA}$",
                                     c=colors[where], ax=ax)
                        sns.lineplot(x=trendfehdwarfs, y=stdnltedwarfs, linestyle="--", c=colors[where], ax=ax)

                    else:
                        sns.lineplot(x=trendfehgiants, y=stdltegiants, label=linestr + "$\mathrm{\AA}$",
                                     c=colors[where], ax=ax)
                        sns.lineplot(x=trendfehgiants, y=stdnltegiants, linestyle="--", c=colors[where], ax=ax)
            ...
            if "dwarf" in startype:
                ...
                if "1" in element:
                    plt.title("Giants")
    plt.savefig("graphs/lineabund.png", bbox_inches="tight")
    plt.show()


sns.color_palette("dark")

def ion_imbalance_fe_split_black_minusredclump():
    verbose = True
    heatmap = True
    fig = plt.figure()
    fig.set_figwidth(15)
    ...
    for element in ["Ti1"]:
        ...
        for lteval in [True, False]:
            ...
            giantfeh = []
            giantion = []
            rc = np.where(np.logical_and(np.logical_and(np.asarray(logg_list)<=3.5, abs(np.asarray(teff_list)-5000)> 500), np.asarray(feh_list) > -0.6))[0]
            giants = np.where(np.logical_and(np.asarray(logg_list)<=3.5, np.asarray(feh_list) <= -0.6))[0]
            giants = list(giants)+(list(rc))
            dwarfs = np.where(np.asarray(logg_list)<=3.5)
            dwarffeh=(np.asarray(feh_list)[dwarfs])
            dwarfion=(np.asarray(imbalance_list)[dwarfs])
            giantfeh=(np.asarray(feh_list)[giants])
            ...
            for y in np.arange(min(feh_list), max(feh_list), 0.1):
                windwarf = np.where(np.logical_and(np.logical_and(y - 0.05 <= feh_list, feh_list <= y + 0.05), np.asarray(logg_list) <=  3.5))[0]
                ...
                if not len(windwarf) < 15:
                    ...
                    trenddwarfs.append(winmeandwarf)
                if verbose:
                    print("dwarf, lteval?, metallicity, mean imbalance, number of stars", lteval, y, len(windwarf))
                ...
                np.where(
                    ...
                    np.logical_and(
                        np.logical_and(
                            y - 0.05 <= feh_list, feh_list <= y + 0.05),
                        np.asarray(logg_list) <= 3.5),
                        np.asarray(abs(np.asarray(teff_list)-5000) > redclum))
                )[0]
                ...
                if not len(wingiant) < 15:
                    ...
                    trendgiants.append(winmeangiant)
                if verbose:
                    print("giant, lteval?, metallicity, mean imbalance, number of stars", lteval, y,
                          len(wingiant))
        ...
        #pkl.dump([feh_list, nlte_imbalance_list, lte_imbalance_list, logg_list], open("ion_imbalance_feh_nlte_lte2.pkl", "wb"))
            ...
            for star in ['giant', 'dwarf']:
                ...
                if star == "giant":
                    ...
                    sns.lineplot(x=trendfehgiants, y=trendgiants, linestyle="-", color=clr, label = ltestring+" non-RC ("+str(len(giantfeh))+")", linewidth =2)
                    ...
                    sns.lineplot(x=trendfehdwarfs, y=trenddwarfs, linestyle="-", color=clr, ax=ax, label = ltestring+ " giants ("+ str(len(dwarffeh))+")")
                    sns.lineplot(x=trendfehgiants, y=trendgiants, linestyle="--", color="red", label = ltestring+" non-RC ("+str(len(giantfeh))+")", linewidth =2)
...
ion_imbalance_fe_split_black()
"""for line in ["4758", "4759", "4778", "4781", "4797", "4801", "4820", "5689", "5716", "5720", "5739", "5866", "6716", "7852"]:
    wl = int(line)
    galactic_evolution_trendlines_fe_line(wl)"""