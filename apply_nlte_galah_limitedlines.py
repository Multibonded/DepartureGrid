...
def solar_twin_lines(element, all, linekeep):
    ...
    r_est_dr2 = np.asarray([i[5] for i in b.values()])
    snr_c2_iraf = np.asarray([i[4] for i in b.values()])
    print(len(c), len(b))
    print(max(snr_c2_iraf))
    exit()
...
def turn_ti_tih(element):
    ...
    solar_values, correction = solar_twin_lines(element, loadall, linekeep)
    for starn in full_galah_nlte:
        for line in full_galah_nlte[starn]:
            ...
            if "1" in element:
                solar_line = [solarlines[lineloc], solarlines[lineloc]+0.05]

            else:

                solar_line = [solarlines[lineloc], solarlines[lineloc]]
            #print(starn, line, solar_line, full_galah_nlte[starn][line][-3])
