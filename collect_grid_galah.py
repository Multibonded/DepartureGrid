...
for windex in range(25):
    ...
    data=pd.read_csv('{}{}_lines_galah'.format(element,ion),dtype='str')
            ...
            for f in fg:
                for x in xg:
                    for a in ag:
                        ...
                        if i.size:
                            """if f == -4 or f == -5:
                                print(i, ind, lte_ew[i], t, g, f, x, a)"""

                            if lte_ew[i] > 1E-4:
                                """solar = False
                                reduced = np.log10(float(lte_ew[i]/1000)/float(line))
                                print(line, lte_ew[i], reduced, ind, t, g, x, f, a)

                                if abs(t - 5772) <= 100 and abs(g - 4.438) <= 0.1 and abs(x - 1) <= 0.2 and abs(
                                        f) <= 0.1:
                                    print(line, lte_ew[i], reduced, ind, t, g, x, f, a)
                                    solar = True

                                if reduced > -4.6 and reduced_flag:
                                    if solar and reduced < -4.7:
                                        pass
                                    else:
                                        ind +=1
                                        wl[np.where(tg == t), np.where(gg == g), np.where(fg == f), np.where(
                                            xg == x), np.where(ag == a)] = -98
                        ...
                        if i.size:
                            ...
                            if nlte_ew[i] >  1E-4 :
                                ...
                                if abs(t - 5772) <= 100 and abs(g - 4.438) <= 0.1 and abs(x - 1) <= 0.2 and abs(
                                        f) <= 0.1:
                                    print("nlte", line, nlte_ew[i], reduced)
                                    solar = True
                                ...
                                if reduced > -4.6 and reduced_flag:
                                    if solar and reduced < -4.7:
                                        ...
                                        ind +=1
                                        wn[np.where(tg == t), np.where(gg == g), np.where(fg == f), np.where(
                                            xg == x), np.where(ag == a)] = -98
    ...
    print(tg.shape)
    outfile=open("grids/{}{}/{}{}_{}_galah.pkl".format(element, ion, element,ion,wg[windex]),"wb")
