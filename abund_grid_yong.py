        ...
        for grav in range(len(wn[0])):
            ...
            for met in range(len(wn[0][0])):
                ...
                for vmic in range(len(wn[0][0][0])):
                    ...
                    if np.all(nlte_ew==-98) or np.all(np.isnan(nlte_ew)):
...
print(len(abund_dict))
if not just_plot_simple:
    pkl.dump(abund_dict, open(element+"nlte_impact_dict_yong.pkl", "wb"))
plt.show()
