...
data=pd.read_csv('{}_lines_yongsneden'.format(element),delim_whitespace=True)
...
for w in wg:
    ...
    if reduced:
        filename=glob.glob('grids/{}/{}_{}_sneden_reduced.pkl'.format(element, element,w))
    else:
        filename = glob.glob('grids/{}/{}_{}_sneden.pkl'.format(element, element, w))
...
grid["wn"]= wn
if reduced:
    outfile=open("grids/{}_sneden_reduced.pkl".format(element),"wb")
else:
    outfile = open("grids/{}_sneden.pkl".format(element), "wb")
...
max = []
nlte = 0
with open('grids/{}_yongsneden.txt'.format(element), 'w') as file:
    sys.stdout = file
    for g in gg:
        for t in tg:
            if (g==0.0) & (t>=4500): 
